            ...
            if line_chunk.srcs:
                ...
                if "動詞" in pos_dst:
                    ...
                    for idx_srcs in line_chunk.srcs:
                        if int(batch_chunk[int(idx_srcs)].dst) == -1:
                            continue
                        text_dst = [line_morphs.surface for line_morphs in batch_chunk[int(idx_srcs)].morphs if line_morphs.pos == "助詞"]
                        if text_dst:
                            temp_pp.append(" ".join(text_dst))
                            temp_para.append("".join(line_morphs.surface for line_morphs in batch_chunk[int(idx_srcs)].morphs))
                    if temp_pp:
                        print(
                            "{}\t{}\t{}".format(
                                verb_dst,
