    ...
    for batch_chunk in list_cunk_neko[:10]:
        for line_chunk in batch_chunk:
            if line_chunk.dst != -1:
                ...
                pos_src = [line_morphs.pos for line_morphs in line_chunk.morphs]
                text_dst = ''.join([line_morphs.surface for line_morphs in batch_chunk[int(line_chunk.dst)].morphs if line_morphs.pos != "記号"])
                pos_dst = [line_morphs.pos for line_morphs in batch_chunk[int(line_chunk.dst)].morphs if line_morphs.pos != "記号"]
